...
@util_router.message(Command(commands='help'), StateFilter(FSMCorpus.send_docx_file))
...
@util_router.message(Command(commands='help'), StateFilter(FSMCorpus.stats_word_input))
async def process_help_select_word(message: Message):
    await message.answer(
        text=LEXICON_RU["help_input_word"]
    )
